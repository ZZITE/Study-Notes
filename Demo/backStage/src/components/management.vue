...
<!-- 订单管理 -->
  	<div class="gement">
    	<div class="gement-header">
      		<div style="float:left;" class="block">
        		<span class="demonstration"></span>
        		<label style="margin-left:20px;color:#747474;" for="">订单搜索:
        			<el-input style="margin-left:20px;" placeholder="订单编号、电话号码" icon="search" v-model="input2" :on-icon-click="handleIconClick"></el-input>
        		</label>
        		<el-date-picker v-model="value4" type="datetimerange" :picker-options="pickerOptions2" placeholder="选择时间范围" align="right"></el-date-picker>
      		</div>
      		...
      		<div id="div1" class="div1">
         		<h2>{{ scron }}</h2>
          		<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
					...
					<el-date-picker v-model="value44" type="datetimerange" :picker-options="pickerOptions2" placeholder="选择时间范围" align="right"></el-date-picker>
					</el-form-item>
					...
					<el-form-item label="用 户 名：" prop="mesg">
					<el-input v-model="ruleForm.mesg"></el-input>
					</el-form-item>
					<div class="typer">
						<el-form-item label="订单状态：" prop="type">
							<el-checkbox-group v-model="ruleForm.type">
								<el-checkbox label="未确定" name="type"></el-checkbox>
								<el-checkbox label="未付款" name="type"></el-checkbox>
								<el-checkbox label="已付款" name="type"></el-checkbox>
								<el-checkbox label="处理中" name="type"></el-checkbox>
								<el-checkbox label="已配送" name="type"></el-checkbox>
								<el-checkbox label="订单完成" name="type"></el-checkbox>
								<el-checkbox label="订单作废" name="type"></el-checkbox>
							</el-checkbox-group>
						</el-form-item>
			...
			<div class="ation-nav">
				...
				<el-tab-pane label="全部订单" name="first">
					...
					<span style="float:left;width:16.5%;text-align:right;">收获地址<img style="margin-left:8px;" src="static/images/shang.png" alt=""><img src="static/images/xia.png" alt=""></span>
					<span style="float:left;width:18.2%;text-align:right;">金额<img style="margin-left:8px;" src="static/images/shang.png" alt=""><img src="static/images/xia.png" alt=""></span>
					<span style="float:left;width:8.5%;text-align:right;">下单时间 <img style="margin-left:8px;" src="static/images/shang.png" alt=""><img src="static/images/xia.png" alt=""></span>
					<span style="float:left;width:9.5%;text-align:right;">订单状态<img style="margin-left:8px;" src="static/images/shang.png" alt=""><img src="static/images/xia.png" alt=""></span>
					<span style="float:left;width:8.8%;text-align:right;">支付状态<img style="margin-left:8px;" src="static/images/shang.png" alt=""><img src="static/images/xia.png" alt=""></span>
					<span style="float:left;width:8.8%;text-align:right;">配送状态<img style="margin-left:8px;" src="static/images/shang.png" alt=""><img src="static/images/xia.png" alt=""></span>
					<span style="float:left;width:6.2%;text-align:right;">操作</span>
					</div>
					<ul>
					<li v-for="(order,index) in orders" :key="index">
						<el-checkbox style="float:left;margin-top:10px;" v-model="checked">{{ order.dingdan }}</el-checkbox>
						<span style="width:13%;text-align:right;float:left;margin-top:10px;">{{ order.email }}</span>
						<p style="float:left;overflow: hidden;width:23%;text-align:right;"><span style="display:block;text-align:left;margin-left:18.5%;">{{ order.addr }}</span>{{ order.address }}</p>
						<span style="width:64px;float:left;margin-left:1%;">{{ order.money }}<strong>{{ order.mode }}</strong></span>
						<span style="float:left;width:11.1%;text-align:right;margin-top:10px;">{{ order.time }}</span style="float:left;">
						<span style="float:left;width:4%;text-align:right;margin-top:10px;color:#E54C87;">{{ order.number }}</span>
						<span style="float:left;width:8.8%;text-align:right;margin-top:10px;">{{ order.stateq }}</span>
						<span style="float:left;width:8.8%;text-align:right;margin-top:10px;">{{ order.statet }}</span>
						<div style="float:left;width:9%;text-align:right;" class="bonter">
						...
						<div style="float:left;margin-bottom:10px;height:29px" class="dingxin">
							<h2 style="float:left;font-size:16px;color:#4C4C4C;margin:5px 0 0 96px;">订单备注</h2>
						</div>
				...
				<el-tab-pane label="未确定" name="second">2</el-tab-pane>
				<el-tab-pane label="未付款" name="third">3</el-tab-pane>
				<el-tab-pane label="已付款" name="fourth">4</el-tab-pane>
				<el-tab-pane label="处理中" name="five">5</el-tab-pane>
				<el-tab-pane label="已配送" name="six">6</el-tab-pane>
				<el-tab-pane label="订单完成" name="seven">7</el-tab-pane>
				<el-tab-pane label="订单作废" name="engiht">8</el-tab-pane>
				</el-tabs>
			</div>
		</div>
        ...
        orders: [
          {
            ...
            number: '未准备',
            addr: '赵五 17·06075892',
            address: '福建 厦门市 思明区 厦港街道大学路世贸大厦2222',
            money: '＄20.65  ',
            mode: '银联支付',
...
.gement .el-date-range-picker table {
    width: -2px;  
}
.gement-header {
    float: left;
...
.gement-header button:hover {
    color: #E54C87;
}
.gement-header .el-input__icon+.el-input__inner {
    width: 330px;
    height: 32px;
    border-radius: 0;
    font-size: 12px;
...
.gement-header .el-input__icon {
    right: 20px;
}
.gement-header .el-input {
    width: 330px;
    height: 32px;
    margin-left: 20px;
}
.gement-header button {
    font-size: 14px;
    border: 0;
    outline: 0;
    color: #747474;
...
.typer .el-checkbox-group {
    width: 800px;
}
.typer .el-checkbox__label {
    margin: 0;
}
.typer button {
    margin-top: 22px;
...
.gement-header .ation-nav {
    width: 100%;
    height: 52px;
} 
.gement-header .el-tabs__item:hover {
    text-decoration: underline;
}
.gement-header .el-tabs__item.is-active {
    color: #E54C87;
}
.gement-header .el-tabs__item {
    font-size: 16px;
    color:rgba(116,116,116,1);
}
.gement-header .el-tabs__active-bar {
    height: 0;
}
.gement-header .el-tabs__nav-scroll {
    margin-top: 10px;
}
.class-z {
    width: 100%;
...
.ation-nav li:hover {
    background:rgba(229,76,135,0.08);
}
.ation-nav li:active {
